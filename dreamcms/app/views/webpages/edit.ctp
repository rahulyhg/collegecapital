	...
	<div id="record">
        ...
        <div id="record_header_wrap">
        	<div id="record_header">
        		<div id="record_detail">Edit Page: <?php echo $this->data['Webpage']['id']; ?></div>
        	</div>
			<?php
                echo '<div style="position:relative;top:5px;left:110px;width:300px;float: left;">';
                echo $this->Form->button('Submit', array('type'=>'submit'));
                //echo $this->Form->button('Reset', array('type'=>'reset'));
                $url = array('action'=>'index');
                echo $this->Form->button('Cancel', array('type'=>'button','onclick'=>"window.location='".$this->Html->url($url)."'"));
                echo '</div>';
                echo '<div id="record_detail" style="float: right; padding: 8px 10px 0 0; position: relative; width: auto;">Last modified on: '.date('d-M-Y H:i:s', strtotime($lastModifiedDetails['Webpage']['modified']))." by ".ucwords($lastModifiedDetails['Users']['name']).'</div>';
            ?>
    	</div>
		<?php 
		...
		$pageTypeAttributes=array('legend'=>false,'class'=>'radio','default'=>(((int)$this->data['Webpage']['parent_page_id']==0)?'Parent':'Child'));
		echo $this->Form->radio('pageType',$pageTypeOptions,$pageTypeAttributes);
		echo "</div>";
		echo "<div id='showParentDropDown' style='".(((int)$this->data['Webpage']['parent_page_id']==0)?'display: none;':'display: block;')."'>";
		...
		echo $this->Form->input('parentPage',array('label'=>'Select Parent Page','type'=>'select','options'=>$parentPages,'default'=>(int)$this->data['Webpage']['parent_page_id']));
		echo "</div>";
		...
		echo $this->Form->input('cms_user_id',array('type'=>'hidden','value'=>$session->read('Auth.User.id')));

...
<div class="users form">
	...
	<?php 
		...
		foreach ($stateOptions as $stState){
			$stStates[$stState['Tblstate']['ID']] = $stState['Tblstate']['State'];
                ...
                $currentState = $this->data['User']['street_state_id'];
		echo $this->Form->input('streetSuburb', array('label' => 'Suburb *'));
		echo $this->Form->input('street_state_id', array('label' => 'State (Street Address) *','type' => 'select', 'escape' => false, 'options' => $stStates, 'default' => $currentState));
		...
		foreach ($stateOptions as $ptState){
			$ptStates[$ptState['Tblstate']['ID']] = $ptState['Tblstate']['State'];
                ...
                $currentState = $this->data['User']['postal_state_id'];
		echo $this->Form->input('postal_state_id', array('label' => 'State (Postal Address)','type' => 'select', 'escape' => false, 'options' => $ptStates, 'default' => $currentState));
	...
	?>
		...
		<?php 
				...
				$url = array('contoller'=>'users', 'action'=>'myprofile');

...
class RatesController extends AppController {
	...
	function view() { //no id required as a parameter
		...
		if ($_SESSION['Auth']['User']['group_id']==1){
			...
			$this->set('banks',$this->Users->find('all', array('conditions' => array('Users.group_id = 4'), 'order' => 'Users.name')));
			...
			if (isset($_GET['group']) && (int)trim($_GET['group'])>0){
				// CHANGE REQUEST Nyree 13/11/2014
				$this->set('rates', $this->Rate->find('all', array(
						'joins' => array(
							array(
								'table' => 'users',
		...
		if($_SESSION['Auth']['User']['group_id']==2 || $_SESSION['Auth']['User']['group_id']==3){
			...
			if (!empty($this->data)) {
				...
				if ($this->Rate->save($this->data)) {
					if(isset($this->data['Rate']['notify']) && $this->data['Rate']['notify']==1 && isset($this->data['Rate']['live']) && $this->data['Rate']['live']==1) {
						$this->_userRatesNotificationMail(); //send notification email
					}
	...
	function edit($id = null) {
		if($_SESSION['Auth']['User']['group_id']==2 || $_SESSION['Auth']['User']['group_id']==3){
			...
			if (!empty($this->data)) {
				if ($this->Rate->save($this->data)) {
					if(isset($this->data['Rate']['notify']) && $this->data['Rate']['notify']==1 && isset($this->data['Rate']['live']) && $this->data['Rate']['live']==1) {
						//$this->set('category',$this->Rate->query("select category from rates_categories where id=".$this->data['Rate']['category_id'].";"));
						//echo $this->data['Rate']['category'];
						$this->_userRatesNotificationMail(); //send notification email
					}
			...
			$this->loadModel('Users');
			$this->set('banks',$this->Users->find('all', array('conditions' => array('Users.group_id = 4'))));
			$this->set('brokers',$this->Users->find('all', array('conditions' => array('Users.group_id = 3'))));
	...
	//send basic email
	function _userRatesNotificationMail() {    
		$this->loadModel('Users');
		$this->set('brokers',$this->Users->find('all', array('conditions' => array('Users.group_id = 3'))));
		//echo "<!-- "; var_dump($this); echo " -->"; // TEST
		$email_list = array();
//		foreach($this->viewVars['brokers'] as $k => $v) {
//			$email_list[] = $v['Users']['email'];
//		}
		$email_list[] = 'yorick@echo3.com.au'; // for test puposes
		//$this->Email->to = array('test@echo3.com.au');		// TEST
		//$this->Email->bcc = array('yorick@echo3.com.au');	// TEST
		$this->Email->to = array('no-reply@collegecapital.com.au');	//change when Go Live		LIVE
		$this->Email->bcc = $email_list;						// array of all brokers		LIVE
		$this->Email->subject = 'College Capital: Rates notice update';    
		$this->Email->replyTo = 'no-reply@collegecapital.com.au';  //change when Go Live  
		$this->Email->from = Configure::read('Company.corporateEmail');   //change when Go Live 
		$this->Email->template = 'rates_notification'; // note no '.ctp' 
...
//		echo "<!-- "; var_dump($this->Email); echo " -->"; // TEST
		$this->Email->send(); // INITIATE WHEN LIVE
	}
}
?>
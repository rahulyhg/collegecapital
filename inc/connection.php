...
if ($_SERVER["SERVER_NAME"]=="echo00"){
	...
	$db_database = "wwwcolle_live20150706";
	...
	$ccap_url	 = "http://localhost/dreamcms/";
}
else {
...
function getDetailsFromUserID($uID){
...
function updateUserLatAndLng($uID, $lat, $lng){
	$sql = "UPDATE `users` set longitude= $lng, latitude = $lat WHERE id=$uID";
	//echo $sql;
	return mysql_query($sql);
}
...
function getAllUsersByLatAndLng($groupid, $lat, $lng){
    $distance = 500; // 5km radius
    $earthRadius = '6371.0'; // In km
    $latitudeRad = deg2rad($lat); 
    $longitudeRad = deg2rad($lng);
    ...
    $sql = "SELECT * FROM (SELECT ROUND($earthRadius * ACOS(SIN($latitudeRad) * SIN( latitude* PI()/180 )
                    ...
                    longitude                 
                FROM
                    `users` WHERE latitude IS NOT NULL AND group_id = $groupid AND live = 1
                ORDER BY
                    distance ASC LIMIT 0,10) AS USERS  where distance < $distance; 
                ";

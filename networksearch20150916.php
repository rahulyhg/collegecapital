	...
	if( !empty($_GET['id']) ){		
		...
		if($userID > 0){
			...
			while($rowUsers = mysql_fetch_array($userDetails, MYSQL_ASSOC)){
				...
				$user['googleMap'] = @$rowUsers['googleMap'];   
                                ...
                                $brokerLocations[] = array('lat' => $latitude, 'lng' => $longitude);
			}
		} else {
			$err = true;	
		}
	    ...
	    while($rowUsers = mysql_fetch_array($allUsers, MYSQL_ASSOC)){
			...
			$user['logo'][] = $rowUsers['logo'];
                        ...
                        if($maxDistance < $rowUsers['distance']){
                            $maxDistance = $rowUsers['distance'];
                        }
                        ...
                        $brokerLocations[] = array('lat' => $rowUsers['latitude'], 'lng' => $rowUsers['longitude']);
		}		
		$err = false;
            }
            else{
...
<script type="text/javascript" language="javascript">
	$(document).ready(function(){
        ...
        var brokerLocations = <?php echo json_encode($brokerLocations); ?>; 
        var distance = parseFloat("<?php echo $maxDistance;?>");
        //alert(brokerLocations.length);
        var zoomLevel = GetZoomLevel(distance);
        //alert(distance + ' => ' + zoomLevel);
       
    // any value between 96 and 159 => See top function       
    if(brokerLocations.length >= 1) {
    var mapOptions = {
                            zoom: zoomLevel,
                            mapTypeId: google.maps.MapTypeId.ROADMAP
                     }                        
                        var latlng = new google.maps.LatLng(brokerLocations[0].lat, brokerLocations[0].lng);
                        var map;
        ...
        for(var i = 0; i < brokerLocations.length; ++i) {
        var latlng = new google.maps.LatLng(brokerLocations[i].lat, brokerLocations[i].lng);     
	...
	<?php       
		if(isset($user['name']) && count($user['name'])>0){
			...
			for($i=0;$i <count($user['name']); $i++){
                            echo "<tr>";
                            echo "<td style='width:15%;'><a href='$site_path/networksearch?id={$user['id'][$i]}'>{$user['name'][$i]} {$user['surname'][$i]}</a></td>";
                            echo "<td style='width:15%;'>{$user['companyName'][$i]}</td>";
                            echo "<td style='width:20%;'><a href='$site_path/networksearch?id={$user['id'][$i]}'>{$user['streetAddress'][$i]}, {$user['streetSuburb'][$i]}</a></td>";
